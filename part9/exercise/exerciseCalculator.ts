...
const calculateExercises = (hours: Array<number>, dailyTarget: number): Result => {
  const average = hours.reduce((x, sum) => sum + x, 0) / hours.length;
  const ratingDescriptions = ['not very good', 'not bad but could be better', 'good', 'excellent', 'amazing'];
